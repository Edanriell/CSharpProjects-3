...
namespace UpdatingDatabaseTable
{
    public class GlobalFactoryService : IDisposable
    {
        ...
        {
            var manufacturers = names.Select(
                name => new Manufacturer() { Name = name, Country = "USA" }
            );
        ...
        {
            var manufacturersInUsa = _context.Manufacturers.Where(m => m.Country == "USA").ToList();

            foreach (var product in products)
            {
                manufacturersInUsa.ForEach(
                    m => m.Products.Add(new Product { Name = product.Name, Price = product.Price })
                );
            }
        ...
        {
            var anyProductInUsa = _context.Products.FirstOrDefault(
                p => p.Manufacturer.Country == "USA"
            );
        ...
        {
            var manufacturersFromUsa = _context.Manufacturers
