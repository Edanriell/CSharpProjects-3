using TreasuryFlightDataAnalysis;

class Program
{
    public static void Main()
    {
        const string FlightDataUrl =
            "https://www.gov.uk/government/uploads/system/uploads/attachment_data/file/245855/HMT_-_2011_Air_Data.csv";
        ...
        Console.WriteLine(
            $"Commands: {GoCommand} | {ClearCommand} | {ClassCommand} value | {OriginCommand} value | {DestinationCommand} value"
        );
        string input;
        do
        {
            ...
            switch (command)
            {
                case GoCommand:
                    ...
                    if (flights.Any())
                    {
                        ...
                        Console.WriteLine(
                            $"Results: Count={flights.Count}, Avg={average:N2}, Min={min:N2}, Max={max:N2}"
                        );
                    }
                    else
                    {
